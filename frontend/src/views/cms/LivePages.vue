    ...
    <div>
      <div class="row">
        <div class="col d-flex justify-content-between">
          <h1 class="h3 text-gray-800">Pages live <span id="greenDot"></span></h1>
        </div>
      </div>
      ...
      <table class="table mt-4">
        ...
        <tbody>
          <tr v-for="(file, index) in files" :key="index">
            ...
            <td><span class="text-muted">{{ $moment(file.createdAt).fromNow() }} by ...</span></td>
            <td>
  ...
  export default {
    setup() {
      ...
      const getFiles = async () => {
        const response = await fetch(`http://localhost:3001/api/cms/templates/list?status=live`, {
